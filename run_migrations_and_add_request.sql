-- ============================================
-- TÜM MİGRATİON'LAR VE ÖNERİ EKLEME (TEK DOSYADA)
-- ============================================
...
add column if not exists daily_water_goal_ml numeric default 2000;

alter table public.profiles
add column if not exists water_reminder_enabled boolean default true;

alter table public.profiles
add column if not exists water_reminder_interval_minutes int default 120;
...
-- Mert Demir kullanıcısını bul ve öneriyi ekle
DO $$
DECLARE
  mert_user_id uuid;
  rec RECORD;
BEGIN
  ...
  IF mert_user_id IS NULL THEN
    RAISE NOTICE 'Mert Demir kullanıcısı bulunamadı. Mevcut kullanıcılar:';
