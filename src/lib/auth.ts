...
import { queryOne, execute, testConnection } from "./turso";
...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    ...
    ...(GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET
      ...
      : []),
    // Credentials Provider - Sadece Turso kullan
    CredentialsProvider({
      name: "credentials",
      credentials: {
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          if (!isConnected) {
            console.error("❌ Turso connection failed in authorize");
            return null;
          }
    ...
    async signIn({ user, account, profile }) {
      ...
      if (account?.provider === "google" && user?.email) {
        try {
          const isConnected = await testConnection();
          if (!isConnected) {
            console.warn("⚠️ Turso not connected, skipping user creation");
            return true; // JWT-only mode devam et
          }
