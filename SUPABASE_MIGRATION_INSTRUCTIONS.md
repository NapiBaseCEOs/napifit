...
## Sorun
`feature_requests` tablosu Supabase'de bulunamıyor. Migration'ı manuel olarak uygulamanız gerekiyor.
...
alter table public.profiles 
  ...
  add column if not exists show_community_stats boolean default true;
...
-- Sadece kendi önerisini güncelleyebilir
...
4. **Doğrulama:**
   ...
   AND table_name IN ('feature_requests', 'feature_request_likes');
   ```
...
- Migration `IF NOT EXISTS` kullanıyor, bu yüzden tekrar çalıştırmak güvenli.
- `set_updated_at()` fonksiyonu zaten mevcut olmalı (önceki migration'larda oluşturulmuş olmalı).
- Eğer `set_updated_at()` fonksiyonu yoksa, önce onu oluşturmanız gerekebilir.

## Test

Migration uygulandıktan sonra:
1. Uygulamaya gidin: https://napibase.com/community
2. "+ Öner" butonuna tıklayın
3. Formu doldurup gönderin
4. Artık hata almamanız gerekir.
